...
<script>
	...
	export default {
		...
		mounted: function() {
			...
			this.$refs.map.focus({
				x: 513101.29403454176, y: 308799.34506960167
			},0)
		...
		methods: {
			...
			loadData: function(key) {
				...
				getGDPIncome(key).then((res) => {
                    ...
                    //
					// });
					that.$refs.CrystalNum.init(`CBD${position}区历年GDP（万元）`,that.mySort(res.data.data));

				});
				//   GDP占比
				getGDPZB(key).then(function(res) {
					that.$refs.LiquidFill.init(`CBD${position}区历年GDP在朝阳区的占比（%）`,  that.mySort(res.data.data));
				})
				// 历年税收收入占比
				getTaxIncomeZB(key).then((res) => {

					that.$refs.TaxIncomeZB.init(`CBD${position}区历年税收收入在朝阳区的占比（%）`,that.mySort(res.data.data) );
				});
				//历年税收收入
				 getTaxIncome(key).then((res) => {
				 	that.$refs.TaxIncome.init(`CBD${position}区历年税收收入（万元）`,that.mySort(res.data.data) )
				 });
				/*历年资产规模*/
				getAssetSize(key).then((res) => {
					that.$refs.AssetSize.init(`CBD${position}区历年资产规模（万元）`,that.mySort(res.data.data) );
				});
				/*资产规模占比*/
				getAssetSizeZB(key).then((res) => {
					that.$refs.AssetSizeZB.init(`CBD${position}区历年资产规模在朝阳区的占比（%）`,that.mySort(res.data.data) );
				});
			...
			tabBoundary: function(i) {
				switch (Number.parseInt(i)) {
					...
					case 0:
						...
						this.$refs.map.focus({
							x: 512073.51303742867,
							y: 306199.03980167717
						},1);
						break;
					//	中心区
					case 1:
						...
						this.$refs.map.focus({
							x:509668.67071527493,
							y: 305201.85252493096
						},2);
						break;
					//	朝阳区
					case 2:

						this.$refs.map.boundary = BOU_QU_CY.features[0];
						// this.$refs.map.setMask();
						this.$refs.map.focus({
							x: 513101.29403454176, y: 308799.34506960167
						},0)
