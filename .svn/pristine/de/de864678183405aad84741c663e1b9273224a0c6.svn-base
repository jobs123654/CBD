<template>
  <div :class="$style.root" v-show="list.name">
    ...
    <div :class="$style.title">
      <p>{{ list.name }}</p>
      <span @click="close()">
        <Icon type="ios-backspace-outline" title="关闭" style="font-size: 20px"/>
      </span>
    </div>
    ...
    <div v-for="(item, i) in list.children" :key="i">
      ...
      >
        <img src="../../assets/images/point@.png" alt v-if="i === itemIndex">
        <img src="../../assets/images/point.png" alt v-else>
        {{ item.name }}
      </span>
      <CheckboxGroup v-model="checkBoxList" @on-change="changeBox">
        ...
        >
          <Checkbox :label="type.name" @on-change="checkData(type)">{{ type.name }}</Checkbox>
          <span :class="$style.color" v-show="type.color">
...
export default {
  ...
  methods: {
    ...
    async showThirdList(i) {
      ...
      if (this.list.children[i].name === "楼宇数量") {
        this.$store.commit("showBuildings", true);
      } else {
      ...
      if (this.list.name === "企业迁徙") {
        ...
        this.$store.dispatch("addMigrateObjs");
        return;
      }
    ...
    queryJiexiangData() {
      ...
      let foreignTableName = this.list.foreignTableName,
        ...
        fields = this.list.fields
          .slice(1)
          .concat(this.list.children[i].queryFields),
        url = this.list.map_url;
      let joinItem = new SuperMap.JoinItem({
        foreignTableName: foreignTableName,
        joinFilter: joinFilter,
    ...
    getPolygon() {
      ...
      return L.polygon(
        this.boundary.geometry.coordinates[0].map(e => [e[1], e[0]])
      );
    }
  }
};
